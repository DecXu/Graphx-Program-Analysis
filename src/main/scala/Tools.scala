...
object Tools {
  ...
  def sum_edge(k: Pegraph): Int = {
    var sum = 0
    for (tmp <- k.getGraph().values) sum += tmp.getSize()
    sum
  }

  def update_graphstore(result: GraphStore, msg: Msg) = {
    for (key <- msg.keySet) result += (key -> (msg(key)._2, msg(key)._3))
  }
  ...
  def extractSubGraph(graph: Pegraph, args: Array[VertexId], len: Int, ret: VertexId, grammar: Grammar): Pegraph = {
    if (len == 0 && ret == -1) return new Pegraph()
  ...
  def getPartial(current: Stmt, pred: Stmt, pred_graph: Pegraph, grammar: Grammar, graphstore: GraphStore): Pegraph = {
    ...
    if (pred.getType() == TYPE.Callfptr) {
      ...
      else out = extractSubGraph(pred_graph, callfptrstmt.getArgs(), callfptrstmt.getLength(), callfptrstmt.getRet(), grammar);
    }
    else if (pred.getType() == TYPE.Call) {
      val callstmt = pred.asInstanceOf[Stmt_call]
      if (current.getType() == TYPE.Return) out = pred_graph
      else out = extractSubGraph(pred_graph, callstmt.getArgs(), callstmt.getLength(), callstmt.getRet(), grammar);
    }
    else if (current.getType() == TYPE.Return) {
      val returnstmt = current.asInstanceOf[Stmt_return]
      if (returnstmt.getLength() == 0) out = new Pegraph()
      else out = extractSubGraph_exit(pred_graph, returnstmt.getArgs(), returnstmt.getLength(), returnstmt.getRet(), grammar, graphstore)
    }
    else out = pred_graph
    out
  }
