...
class ComputationSet {
  ...
  private val Olds =  mutable.Map.empty[VertexId, EdgeArray2]
  private val Deltas =  mutable.Map.empty[VertexId, EdgeArray2]
  private val News =  mutable.Map.empty[VertexId, EdgeArray2]
  ...
  def init_add(out: Pegraph, m: mutable.Map[VertexId, EdgeArray2], isConservative: Boolean) = {
    ...
    if(isConservative){
      ...
      //OldsV <- out
      for(it <- out.getGraph()){
        setOlds(it._1, it._2.getSize(), it._2.getEdges(), it._2.getLabels())
      }
    }
  }

  def print_graph_map(graph: mutable.Map[VertexId, EdgeArray2]): String = {
    var size = 0
    var str = ""
    for(it <- graph){
      str += (it._1 + " -> " + it._2 + '\n')
