class Grammar extends Serializable
{
  def getLabelValue(str: Array[Byte]): Byte = {
    for (i <- 0 until numRawLabels) {
      if (rawlabel(i) sameElements str) return (i - 128).toByte
    }
    127
  }
  ...
  final def addRawLabel(label: Array[Byte]): Int = {
    for (i <- 0 until numRawLabels) {
      if (rawlabel(i) sameElements label) {
        return i
      }
    }
  ...
  @inline final def changeShort(a: Byte, b: Byte): Short = (a.toShort << 8 | (b.toShort & 0xFF)).toShort
  ...
  final def loadGrammar(s: String): Unit = {
    ...
    val array = s.split('\t')
    for (i <- array.indices) arg(i) = array(i).toCharArray.map(x => x.toByte)
    for (i <- array.indices) index(i) = addRawLabel(arg(i))
    array.length match{
      case 1 => {
  ...
  @inline final def isEruleLabel(label: Byte): Boolean = {
    for (i <- 0 until getNumErules()) {
      if (label == getErule(i)) return true
    }
    false
  }
}
